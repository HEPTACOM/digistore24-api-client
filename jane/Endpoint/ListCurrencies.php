...
class ListCurrencies extends \Jane\OpenApiRuntime\Client\BaseEndpoint implements \Jane\OpenApiRuntime\Client\Psr7Endpoint
{
    ...
    {
        return str_replace(['{apiToken}'], [$this->apiToken], '/{apiToken}/json/listCurrencies');
    }
    ...
    protected function getQueryOptionsResolver(): \Symfony\Component\OptionsResolver\OptionsResolver
    {
        ...
        $optionsResolver->setDefined(['operator', 'language', 'convert_to']);
        $optionsResolver->setRequired([]);
        $optionsResolver->setDefaults([]);
        $optionsResolver->setAllowedTypes('operator', ['string']);
        $optionsResolver->setAllowedTypes('language', ['string']);
        $optionsResolver->setAllowedTypes('convert_to', ['string']);
     ...
     * {@inheritdoc}
     *
     * @return \Heptacom\DigiStore24\Model\ListCurrenciesResponse|\Heptacom\DigiStore24\Model\ErrorResponse|null
     */
    protected function transformResponseBody(string $body, int $status, \Symfony\Component\Serializer\SerializerInterface $serializer, ?string $contentType = null)
    {
        if ($status === 200 && mb_strpos($contentType, 'application/json') !== false) {
            return $serializer->deserialize($body, 'Heptacom\\DigiStore24\\Model\\ListCurrenciesResponse', 'json');
        }
